    ...
    const createOptions = (roles, parentContainer) => {
        roles.forEach(role => {
            ...
            if (savedVoiceKey) {
                ...
                const systemDefaultVoice = filteredVoices.find(voice => voice.default === true);
                select.value = systemDefaultVoice ? createVoiceKey(systemDefaultVoice) : (filteredVoices.length > 0 ? createVoiceKey(filteredVoices[0]) : null);
            }
