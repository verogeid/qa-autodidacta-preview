    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const fragment of fragments) {
            if (fragment.silent) {
                ...
                // Aplicamos la cursiva solo si es un discurso de personaje y no es una voz narrativa por defecto.
                if (isCharacterSpeech && fragmentRole !== 'voice-4' && fragmentRole !== 'voice-5') {
...
function escapeRawTags(text) {
    // Escapar los signos de menor y mayor que, fundamentales para HTML.
    let escapedText = text.replace(/</g, "&lt;"); // [1, 2, 3, 4]
    escapedText = escapedText.replace(/>/g, "&gt;"); // [1, 2, 3, 4]

    // Escapar los caracteres adicionales solicitados, usando entidades numéricas.
    // Nota: Algunos de estos caracteres no son estrictamente necesarios de escapar en HTML de forma general,
    // pero se hace para la compatibilidad específica con tu parser.
    escapedText = escapedText.replace(/#/g, "&#35;"); // [3]
    escapedText = escapedText.replace(/\$/g, "&#36;"); // El '$' es un carácter especial en regex, necesita ser escapado
    escapedText = escapedText.replace(/%/g, "&#37;"); // [3]
    escapedText = escapedText.replace(/\(/g, "&#40;"); // El '(' es un carácter especial en regex, necesita ser escapado [3]
    escapedText = escapedText.replace(/\)/g, "&#41;"); // El ')' es un carácter especial en regex, necesita ser escapado [3]
    escapedText = escapedText.replace(/\[/g, "&#91;"); // El '[' es un carácter especial en regex, necesita ser escapado [3]
    escapedText = escapedText.replace(/\]/g, "&#93;"); // El ']' es un carácter especial en regex, necesita ser escapado [3]
...
function formatDisplay(text) {
    let escaped = escapeRawTags(text);
