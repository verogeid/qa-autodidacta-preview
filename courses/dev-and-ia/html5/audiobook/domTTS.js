...
function findBestDefaultVoice(voices, lang) {
    ...
    if (googleVoice) {
        if (debugFlag) console.log(`[domTTS] ⭐ Voz por defecto para "${lang}": Priorizando Google (${googleVoice.name}).`);
        return googleVoice;
    }
    ...
    if (systemDefaultVoice) {
        if (debugFlag) console.log(`[domTTS] ⭐ Voz por defecto para "${lang}": Usando la voz por defecto del sistema (${systemDefaultVoice.name}).`);
        return systemDefaultVoice;
    }

    // 3. Si no hay ninguna de las anteriores, usa la primera de la lista
    if (filtered.length > 0) {
        if (debugFlag) console.log(`[domTTS] ⭐ Voz por defecto para "${lang}": Usando la primera voz disponible (${filtered[0].name}).`);
        return filtered[0];
    }
...
function speakNext() {
    ...
    if (textToSpeak) {
        ...
        const phraseLang = currentPhraseSpan.getAttribute('lang') || 'es'; // Asegura un idioma de fallback
        ...
        // --- INICIO: Lógica añadida para la voz por defecto (Google-first) ---
        if (!selectedVoice) {
            selectedVoice = findBestDefaultVoice(speechState.availableVoices, phraseLang);
        }
        // --- FIN: Lógica añadida ---
